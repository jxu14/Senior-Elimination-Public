...
def send_game_update(game_update):
    server = smtplib.SMTP_SSL('smtp.gmail.com:465')
    server.login(EMAIL, EMAIL_PASSWORD)
    for senior in Senior.objects.filter(email_subscribed=True, is_superuser=False):
        message = MIMEMultipart()
        ...
        message['From'] = EMAIL
        message['To'] = senior.email
        message['Subject'] = f'Senior Elimination - {game_update.created_at.strftime("%A %m/%d at %I:%M %p")} - Game Update!'
        message.attach(MIMEText(content, 'plain'))
        server.send_message(message)
        del message
    server.quit()
...
def send_reassign_targets():
    server = smtplib.SMTP_SSL('smtp.gmail.com:465')
    server.login(EMAIL, EMAIL_PASSWORD)
    for senior in progressbar.progressbar(Senior.objects.filter(elimination__isnull=True, is_superuser=False)):
        message = MIMEMultipart()
...
def send_initial_passwords():
    server = smtplib.SMTP_SSL('smtp.gmail.com:465')
    server.login(EMAIL, EMAIL_PASSWORD)
    for senior in progressbar.progressbar(list(Senior.objects.filter(is_superuser=False))):
        try:
            message = MIMEMultipart()
